        ...
        <div class="row">
            ...
            <div class="col-sm-12 col-md-9 right-content" style="display: inline;">
                ...
                <div class="row" style="margin:0%; background-color:white;">
                    <div class="row" style="margin:0%; background-color:white;">
                        <button class="btn btn-primary" disabled style="padding:1%;"> Your Loans </button>
                        <table class="table table-dark display" id="myTable">
                            <thead>
                                <tr>
                                    <th scope="col" style="color:yellow;"> Loan Id</th>
                            ...
                            <tbody style="overflow-y: scroll;">
                                ...
                                ?>
                                    <tr>
                                        <td style="color:yellow;"> <?= $loan['Id'] ?> </td>
                                        <td> <?= $loan['UserId'] ?> </td>
                                        <td> <?= $loan['Date'] ?> </td>
                                        <td> <?= $loan['Amount'] ?> </td>
                                        <td> <?= $loan['Status'] ?> </td>
                                        <td>
                                            <button type="button" class="btn btn-primary btn-lg" data-bs-toggle="modal" data-bs-target="#editId<?= $loan['Id'] ?>" <?php if($loan['Status']!='Pending'){  ?> disabled <?php } ?>>
                                                Edit
                                            </button>
                                            <div class="modal fade" id="editId<?= $loan['Id'] ?>" tabindex="-1" role="dialog" aria-labelledby="modelTitleId" aria-hidden="true">
                                                <div class="modal-dialog" role="document">
                                                    <div class="modal-content">
                                                        <div class="modal-header">
                                                            <h5 class="modal-title" style="color:black;"> Edit Loan </h5>
                                                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                                        </div>
                                                        <form method="POST" action="admin_editLoan/">
                                                            <div class="modal-body">
                                                                <input type="text" name="loanId" value="<?= $loan['Id'] ?>" hidden>
                                                                <input type="text" name="UserId" value="<?= $loan['UserId'] ?>" hidden>
                                                                <input type="text" name="Amount" value="<?= $loan['Amount'] ?>" hidden>
